function [Region_overlap] = RegionOverlapPixel2(Diff_Projection,Target_May,Target_ID,Potential_Location,MAT_features)
...
for i=1:size(Target_May,1)
    ...
    region=Diff_Projection.*0;
    [x,~]=find(Potential_Location(:,5)==Target_ID);
    Location1=Potential_Location(x,1:2);
    Region(Location1(:,1),Location1(:,2))=1;
    [x,~]=find(Potential_Location(:,5)==Target_May(i,1));
    ...
    if N==1
        ...
        [x,~]=find(Potential_Location(:,5)==Target_min);
        Feature_min=MAT_features(x,:);
        [x,~]=find(Potential_Location(:,5)==Target_max);
