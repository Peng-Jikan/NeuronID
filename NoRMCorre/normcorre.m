...
Interval_length=ceil((T-1)/(init_batch-1));
interval = 1:Interval_length:T;
if interval(end)>T
    interval(end)=T;
end
...
if nargin < 3 || isempty(template)
    fprintf('Select %i frames from the sequence, with each frame %i frames apart, to create a good template....',size(interval,2),Interval_length);
